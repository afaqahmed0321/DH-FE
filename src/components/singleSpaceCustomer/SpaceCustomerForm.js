...
const SpaceCustomerForm = ({ onHide, slots }) => {
  ...
  const [selectedCard, setSelectedCard] = useState(
    userCards.length > 0 ? userCards[0].id : ""
  );
          ...
          {(formik) => (
            <Form>
              ...
              <div className="d-flex align-items-center">
                ...
                <p className="fw-bold ps-2 mt-3">Paying with card</p>
                {cardAlert && (
              ...
              <Row className="justify-content-start gap-sm-0 gap-3">
                  ...
                  userCards.map((card, index) => {
                    return (
                      <Col key={index} md="4" sm="4" className="selection">
                        <div
                          className={`rounded-2 bg-cards pt-1 ps-2 pe-2 ${selectedCard === card.id ? "selected" : ""
                            }`}
                          onClick={() => handleCardClick(card.id)}
                        >
                          <Image
                            src={card.brand === "Visa" ? visacard : mastercard}
                          />
                          <span className="text-14 text-color ps-2">
                            {card.brand}
                          </span>
                ...
                <Col md="4" sm="4">
                  <Button
                    className="btn-blue-outline w-100"
                    onClick={() => setCardAlert(true)}
                  >
                    + Add Card
                  </Button>
                </Col>
